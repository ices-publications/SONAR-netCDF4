...
e|Variables | |
 ...
 |{var}float nominal_transducer_frequency |O |Frequency for which transducer is designed to operate in CW.
 3+|{attr}:long_name = "Nominal transducer frequency" 
 3+|{attr}:units = "Hz" 
 3+|{attr}float :valid_min = 0.0 
 ...
 |{var}float transmit_pulse_slope(ping_time, tx_beam) |O |Slope of the transmit pulse. 
 3+|{attr}:long_name = "Slope of transmitted pulse" 
 3+|{attr}:units = "%" 
 3+|{attr}float :valid_min = 0.0 
 3+|{attr}float :valid_max = 50.0 
...
e|Subgroups | |
 ...
 |{var}VolumeBackscatter|O |Subgroup containing calculated volume backscatter data.
|===

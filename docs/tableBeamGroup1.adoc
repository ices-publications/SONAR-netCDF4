...
e|Variables | |
 ...
 |{var}pulse_t transmit_pulse_model_i(ping_time, tx_beam) |MA |Imaginary part of the model of the transmit pulse. The exact shape of the theoretical transmit pulse is given at the same sampling rate of the backscatter measurements. The shape reflects both the weighting of the pulse and the filters that have been applied. The  pulse shape is used for matched filtering of complex samples in type 4 conversion equations.

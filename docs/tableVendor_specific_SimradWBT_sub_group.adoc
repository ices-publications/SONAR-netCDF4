...
e|Variables | |
 |{var}float filter_coefficients_i(start_time, beam, stage) |M |Imaginary part of filter coefficients.
 3+|{attr}:long_name = "Filter coefficients (imaginary part)" 
  
 |{var}float filter_coefficients_i(start_time, beam, stage) |M |Real part of filter coefficients.
 3+|{attr}:long_name = "Filter coefficients (real part)" 
  
 |{var}int filter_decimation(start_time, beam, stage) |M |Filter decimation.
 3+|{attr}:long_name = "Filter decimation" 
 3+|{attr}int :valid_min = 1 
 
 |{var}int filter_center_frequency(start_time, beam, stage) |M |Centre frequency of the filter.
 3+|{attr}:long_name = "Filter centre frequency" 
 3+|{attr}int :valid_min = 1 
 3+|{attr}:units = "hz" 

 |{var}int filter_bandwidth(start_time, beam, stage) |M |Bandwidth of the filter.
 3+|{attr}:long_name = "Filter bandwidth" 
 3+|{attr}int :valid_min = 1 
 3+|{attr}:units = "hz" 

 |{var}float filter_transitionband(start_time, beam, stage) |M |Transition band of the filter.
 3+|{attr}:long_name = "Filter transition band" 
 3+|{attr}float :valid_min = 1.0 
 3+|{attr}:units = "hz" 

 |{var}int filter_gain(start_time, beam, stage) |M |Gain in the filter.
 3+|{attr}:long_name = "Filter gain" 
 3+|{attr}int :valid_min = 1 

 |{var}int filter_notches(start_time, beam, stage) |M |Number of notches in the filter.
 3+|{attr}:long_name = "Filter notches" 
 3+|{attr}int :valid_min = 0 

 |{var}int filter_correctADSampleTimingInFilter(start_time, beam, stage) |M |Correct AD Sample Timing in the filter.
 3+|{attr}:long_name = "Filter correct AD sample timing" 
 3+|{attr}int :valid_min = 0 

 |{var}int filter_length_in_filter_banks(start_time, beam, stage) |M |Filter length in filter banks.
 3+|{attr}:long_name = "Filter length in filter banks" 
 3+|{attr}int :valid_min = 0 

 |{var}int filter_bits_to_truncate_after_multiplication(start_time, beam, stage) |M |Filter bits to truncate after multiplication.
 3+|{attr}:long_name = "Filter bits to truncate after multiplication" 
 3+|{attr}int :valid_min = 0 

 |{var}float filter_generator_version(start_time, beam, stage) |MA |Version of the filter generator used.
 3+|{attr}:long_name = "Filter generator version" 

 |{var}ping_t ping_valid(start_time) |MA |Reference to pings (by ping_time) using the group variables.
 3+|{attr}:long_name = "Time reference to pings using the variables" 

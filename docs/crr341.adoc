...
ICES WGFAST - Open Data subgroup
:toc: left
:toclevels: 4
:doctype: book
:revnumber: 2.1 (working draft)
:revremark: Generated at {localdatetime}
:xrefstyle: short
...
_ICES boilerplate stuff goes here._
...
... *Beam_groupX –* contains ping based backscatter; the group supports multiple sensors of the same type, e.g. multiple frequencies;
...
The top-level group (labelled "/" in netCDF4 terminology) contains metadata about the SONAR-netCDF4 file, represented as attributes in the group (<<topLevelTable>>).
...
Data from each beam mode (e.g. horizontal and vertical beam modes) are stored in subgroups under the /Sonar group (see <<sonarBeamGroupTable>>). The form of the backscatter data can vary between different sonar systems. For example, some provide a complex-valued amplitude, while others provide a real- or integer-valued amplitude.
...
[.landscape]
<<<
==== Proposed single target additions

This group contains the single target detections from split-aperture beams positioned with their range and split-aperture arrival angles. Target Strength (TS) is calculated using conversion equations detailed in <<Conversion equations>>. Algorithm for single target detection and parameters are detailed in citenp:[soule1997]. The netCDF4 group name is /Sonar/Beam_group/SingleTarget.

.Description of the single_target subgroup of the beam group.
[[sonarSingleTargetTable]]
include::tableSingleTarget_sub_group.adoc[]
[.portrait]
<<<
...
The netCDF4 group name is /Sonar/Beam_groupX/ADCP.
...
The vendor specific group contains information about the sonar and data specific to the sonar. Data in this group must not be necessary for normal quantitative use of the sonar data. The contents of this group are at the discretion of the sonar and software that writes the SONAR-netCDF4 file. The netCDF4 group name is */Vendor_specific*. There is no mandatory information for the vendor specific group.
...
.Description of the SimradWBT group in the Vendor group.
[[sonarVendor_specific_SimradWBT_Table]]
include::tableVendor_specific_SimradWBT_sub_group.adoc[]
...
[stem#eqType1TS,reftext=({counter:eqs})]
++++
\begin{equation}
...
[stem#eqTimeToRange,reftext=({counter:eqs})]
++++
\begin{equation}
...
[stem#eqType1Sv,reftext=({counter:eqs})]
++++
\begin{equation}
...
where latexmath:[\psi] is the equivalent beam angle (equivalent_beam_angle, sr), latexmath:[\tau_e] is the effective pulse duration (receive_duration_effective, s), and _G_ is the transducer gain (transducer_gain, dB).
...
[stem#eqType2TS,reftext=({counter:eqs})]
++++
\begin{equation}
...
where _A_ (backscatter_r, 1) is linearly proportional to the amplitude of the received echo , _r_ is the range as given by equation <<eqTimeToRange>>, and _α_ is the absorption coefficient of sound in water (absorption_indicative, dB/m). _SL_ is source level (transmit_source_level (re 1μPa at 1 m), dB) and _K_ is receiver sensitivity (receiver_sensitivity (re 1/μPa), dB). Both parameters may depend on tilt angle of the beam for transmitting and receiving.
...
[stem#eqType2Sv,reftext=({counter:eqs})]
++++
\begin{equation}
...
[stem#eqType3Pr,reftext=({counter:eqs})]
++++
\begin{equation}
...
[stem#eqType3TS,reftext=({counter:eqs})]
++++
\begin{equation}
...
[stem#eqType3Sv,reftext=({counter:eqs})]
++++
\begin{equation}
...
[stem#eqType3tau_e,reftext=({counter:eqs})]
++++
\begin{equation}
...
[stem#eqType3r,reftext=({counter:eqs})]
++++
\begin{equation}
tbd.
\end{equation}
...
[stem#eqType4y_pc,reftext=({counter:eqs})]
++++
\begin{equation}
...
[stem#eqType4p_rxe,reftext=({counter:eqs})]
++++
\begin{equation}
...
[stem#eqType4Pr,reftext=({counter:eqs})]
++++
\begin{equation}
...
[stem#eqType4tau,reftext=({counter:eqs})]
++++
\begin{equation}
...
[stem#eqType4Sv,reftext=({counter:eqs})]
++++
\begin{equation}
...
[stem#eqType4tau_e,reftext=({counter:eqs})]
++++
\begin{equation}
...
[stem#eqType4y_txpc,reftext=({counter:eqs})]
++++
\begin{equation}
...
[stem#eqType4Svf,reftext=({counter:eqs})]
++++
\begin{equation}
...
[stem#eqType4TSf,reftext=({counter:eqs})]
++++
\begin{equation}
...
[stem#eqFCV38TS,reftext=({counter:eqs})]
++++
\begin{equation}
...
[stem#eqType6I,reftext=({counter:eqs})]
++++
\begin{equation}
...
[stem#eqType6Q,reftext=({counter:eqs})]
++++
\begin{equation}
...
[stem#eqType6A,reftext=({counter:eqs})]
++++
\begin{equation}
...
[stem#eqType6r,reftext=({counter:eqs})]
++++
\begin{equation}
...
[stem#eqCSFAB,reftext=({counter:eqs})]
++++
\begin{equation}
...
[stem#eqCSxyz1,reftext=({counter:eqs})]
++++
\begin{equation}
\begin{bmatrix}
x\\
...
[stem#eqCSR_PCR_SCS,reftext=({counter:eqs})]
++++
\begin{equation}
...
[stem#eqCSRxyz,reftext=({counter:eqs})]
++++
\begin{eqnarray}
...
To be clear, the right-hand rule when applied to roll, pitch, and heading means that:
...
Some sensors (e.g. the position, attitude sensor) can have their installation offset and angle defined in the Platform group and relative to the platform-coordinate system. Those parameters are usually taken into account by the sensors and the sensor data values are considered to be given relative to the platform origin (namely roll, pitch should not be compensated for level arms and installation offset except when explicitely specified).
...
[stem#eqCSti,reftext=({counter:eqs})]
++++
\begin{equation}
...
* Its range stem:[r_i] given by equation <<eqTimeToRange>>.
...
and
[stem#eqCSR_BCS_TCS1,reftext=({counter:eqs})]
++++
\begin{equation}
R_{BCS->TCS} =
\begin{bmatrix}
...
[stem#eqCSR_PCS_SCS1,reftext=({counter:eqs})]
++++
\begin{equation}
...
[stem#eqCST_TCS_PCS1,reftext=({counter:eqs})]
++++
\begin{equation}
...
The position of the echo can be defined as :
[stem#eqCSxyz2,reftext=({counter:eqs})]
++++
\begin{equation}
\begin{bmatrix}
x\\
...
[stem#eqCSR_BCS_TCS2,reftext=({counter:eqs})]
++++
\begin{equation}
R_{BCS->TCS} =
\begin{bmatrix}
...
[stem#eqCSR_TCS_PCS,reftext=({counter:eqs})]
++++
\begin{equation}
...
[stem#eqCST_TCS_PCS2,reftext=({counter:eqs})]
++++
\begin{equation}
T_{TCS->PCS} = \begin{bmatrix}
transducer\_offset\_x \\
...
[stem#eqCSR_PCS_SCS2,reftext=({counter:eqs})]
++++
\begin{equation}
R_{PCS->SCS}(t_i)  = R_{z}(h(t_i)) \cdot R_{y}(p(t_i)) \cdot R_{x}(r(t_i))
\end{equation}
...
[stem#eqTypeA,reftext=({counter:eqs})]
++++
\begin{equation}
...
\end{equation}
++++
where latexmath:[\arctan] is the four-quadrant inverse tangent, latexmath:[y_x] the complex signal from quadrant latexmath:[x], * indicates the complex conjugate, latexmath:[y_\theta] the complex signal for the minor axis, latexmath:[y_\phi] the complex signal for the major axis, and latexmath:[\Re] and latexmath:[\Im] the real and imaginary parts of the complex signals. The angle sensitivities are latexmath:[\eta_\phi] for the major axis and latexmath:[\eta_\theta] for the minor axis.
...
[stem#eqTypeBComega,reftext=({counter:eqs})]
++++
\begin{equation}
...
[stem#eqTypeBCthetaphi,reftext=({counter:eqs})]
++++
\begin{equation}
...
Version 2 of this document mades significant changes to the convention presented in version 1. These are summarised here:

decibel units:: The reference values for decibel units have been moved from the units attribute to the variable description (typically the `long_name`).
...
SingleTarget:: A new subgroup, SingleTarget is added to allow for single target detections using backscatter_r and backscatter_i or backscater_r and echoangle_major and echoangle_minor. Calculation of Target Strength (TS) of single targets is given in equation 2,3,4 and 6 for different sonars.

Provenance:: Changed the required timestamp format to match that used elsewhere in the convention.

Vendor_specific_SimradWBT:: Vendor specific table created with a new subgroup, Vendor_specific_SimradWBT, to provide transducer and transceiver information for processing and reference.
...
|{revnumber}|2.0 | April 2022|Significant additions to support additional sonar types.

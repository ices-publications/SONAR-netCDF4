...
[.landscape]
<<<
==== Proposed calculated backscatter additions

This group contains the calculated latexmath:[S_v], for a single ping or 
a series of pings formed by a calculation interval.
The netCDF4 group name is VolumeBackscatter and is a subgroup of /Sonar/Beam_group.

.Description of the VolumeBackscatter subgroup of the BeamGroup1 group.
[[sonarVolumeBackscatterTable]]
include::tableVolumeBackscatter_sub_group.adoc[]

.Description of the AveragedVolumeBackscatter subgroup of the VolumeBackscatterGroup group.
[[sonarAveragedVolumeBackscatterTable]]
include::tableAveragedVolumeBackscatter_sub_group.adoc[]
[.portrait]
<<<
...
where _A_ (backscatter_r, 1) is linearly proportional to the amplitude of the received echo , _r_ is the range as given by equation \eqref{eq:timeToRange}, and _α_ is the absorption coefficient of sound in water (absorption_indicative, dB/m). _SL_ is source level (transmit_source_level (re 1μPa at 1 m), dB) and _K_ is receiver sensitivity (receiver_sensitivity (re 1/μPa), dB). Both parameters may depend on tilt angle of the beam for transmitting and receiving.
...
SimradWBT:: Vendor specific table created with a new subgroup, Vendor_specific_SimradWBT, to provide transducer and transceiver information for processing and reference.

VolumeBackscatter:: A new subgroup, VolumeBackscatter, to Beam_Group1, has been added to to store calculated SV/SV(f) for single ping, all samples.

AveragedVolumeBackscatter:: A new subgroup, AveragedVolumeBackscatter, to VolumeBackscatter, has been added to to store calculated SV/SV(f) for single ping or over a series of ping, calculation interval, for a selected layer range.

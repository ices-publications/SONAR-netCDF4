...
==== Time units
Even if netcdf and conventions of the UDUNITS-2 allows to specify any valid time unit, to prevent any extra complication to software that reads the SONAR-netCDF4 files only two time units are supported in this convention.

* nanoseconds since 1601-01-01 00:00:00Z

* nanoseconds since 1970-01-01 00:00:00Z

The choice of nanoseconds is due to the sonar system timestamp precision that can be less than one millisecond. Since other forms of units like 100 nanoseconds are not well supported by the various tools that read CF-formatted time units, nanoseconds units is retained.

Epoch time 1601 is because it is the epoch time unit used in Simrad .raw files but also in Windows32/64  systems.

Epoch time 1970 is because it is the epoch time widely used in Unix and most programming langages.
...
The netCDF4 group name is Mean-current and is a subgroup of ADCP group.
